document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndRenderProducts() {
        ...
        try {
            const response = await fetch('/spiceheritage/admin/api/products');
            ...
            products.forEach(product => {
                ...
                row.innerHTML = `
                    <td>${product.id}</td>
                    <td><img src="/spiceheritage/api/product-image?id=${product.id}" alt="${product.name}" class="product-thumbnail"></td>
                    <td>${product.name}</td>
    ...
    async function handleDelete(event) {
        ...
        if (confirm(`Are you sure you want to delete product ID ${productId}?`)) {
            try {
                const response = await fetch(`/spiceheritage/admin/products?action=delete&id=${productId}`, {
